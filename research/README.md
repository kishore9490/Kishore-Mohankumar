...
**Study 4 — Clearinghouse API & healthcare connectivity (cost, coverage, limitations, opportunity)**

| File | What it is |
|---|---|
| `AI_Revenue_Recovery_OS_Clearinghouse_API_Report.pdf` | **40-page investor/technical report** answering: can a clearinghouse API give us most of the data we need, what does it cost, what does it *not* cover, and where do we still need direct payer APIs, FHIR, portals, AI voice and humans. 35 sections — exec summary; clearinghouse vs payer vs API vs FHIR vs portal vs voice vs human; is it free; free-sandbox vs paid-production; deep dives on **Stedi, Availity, Waystar, Optum/Change, Experian, Office Ally, Claim.MD, TriZetto**; EDI transaction matrix (270/271, 276/277, 277CA, 278, 837P/I/D, 835, 999, TA1, 275); eligibility field matrix; benefits; patient responsibility; claims; claim status; ERA; prior auth (Da Vinci PAS/CRD/DTR; CMS-0057-F 2027); denials; appeals (no universal standard); payer policies; payer API; FHIR; portal; AI voice; connectivity orchestration ("don't call when you can connect digitally"); architecture; cost model (5 volume scenarios); MVP/pilot/production; build vs buy vs partner; product opportunity; security/HIPAA; investor summary; final recommendation. Charts, architecture/flow diagrams, TOC, page numbers, source footnotes. |
| `AI_Revenue_Recovery_OS_Clearinghouse_API_Data.xlsx` | **12-sheet research-ready workbook** — Clearinghouse Comparison, Pricing, EDI Transactions, API Availability, FHIR, Payer Coverage, Free vs Paid, Connectivity Costs, MVP/Pilot/Production Cost, Sources. Color-coded (YES/NO/PARTIAL/QUOTE REQUIRED), hyperlinked sources. |

Generators: `clearinghouse_data.py` (researched 2026 clearinghouse profiles + EDI/FHIR/CMS facts + parameterized connectivity cost model), `build_clearinghouse.py` (13 charts/diagrams, 12-sheet Excel, 40-page PDF).

**Study 4 headlines:** A clearinghouse API covers the standardized transactions (eligibility, claims, status, ERA) across thousands of payers via one integration, but **not** universal prior auth, appeals (no universal standard), payer policies, denial reasoning or orchestration. **Free sandbox ≠ free production** (Stedi Basic = 100 free txns/mo; Availity Essentials free for participating payers; most production pricing is usage-tiered or **QUOTE REQUIRED**). MVP connectivity is near-$0 on free/sandbox tiers. **Recommendation: do not build a clearinghouse — build the AI intelligence + orchestration layer above it** (Clearinghouse + Direct Payer APIs + FHIR + Portal + AI Voice + Human, controlled by an AI connectivity orchestrator).
